...
if st.sidebar.checkbox("Enable Real-Time Predictions"):
    if 'model' not in locals():
        st.error("Please train a model first to enable predictions.")
    else:
        ...
        for feature in features:
            st.session_state.input_data[feature] = st.sidebar.number_input(
                f"Input {feature}", value=st.session_state.input_data[feature]
            )
...
if st.sidebar.checkbox("Enable Batch Predictions"):
    if 'model' not in locals():
        st.error("Please train a model first to enable batch predictions.")
    else:
        batch_file = st.sidebar.file_uploader("Upload Batch CSV for Predictions", type="csv")
        if batch_file:
            try:
                batch_data = pd.read_csv(batch_file)
                # Ensure batch data has the necessary features
                missing_features = [feature for feature in features if feature not in batch_data.columns]
                if missing_features:
                    st.error(f"Batch data is missing required features: {', '.join(missing_features)}")
                else:
                    batch_predictions = model.predict(batch_data[features])
                    st.write("Batch Predictions:")
                    st.dataframe(pd.DataFrame({"Prediction": batch_predictions}))
            except Exception as e:
                st.error(f"Error during batch predictions: {e}")

# Feature 7: Export Predictions
if st.sidebar.button("Export Predictions"):
    if 'model' not in locals():
        st.error("Please train a model first to enable export predictions.")
    else:
        try:
            predictions = model.predict(X)  # Ensure X exists and is preprocessed
            ...
            st.error(f"Error during export predictions: {e}")
